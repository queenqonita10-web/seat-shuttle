...
const DriverProfile = () => {
  ...
  const driver = driversData[0]; // current driver
      ...
      <div className={cn(
        ...
        <div className="mx-auto max-w-md">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-2xl font-black uppercase tracking-tight">{driverName}</h1>
              <div className="flex items-center gap-3 mt-1 text-sm opacity-70">
                <span className="flex items-center gap-1 font-bold"><Star size={14} fill="currentColor" /> 4.8</span>
                <span className="flex items-center gap-1 font-bold"><Bus size={14} /> 156 trip</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-bold">{driverPhone}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className={cn("border-0 rounded-2xl", isDrivingMode ? "bg-zinc-900 ring-1 ring-white/10" : "bg-card")}>
            <CardContent className="p-0 divide-y divide-border/10">
              <div className="flex items-center justify-between p-5">
                ...
                <Switch checked={isDrivingMode} onCheckedChange={setIsDrivingMode} />
              </div>
              <div className="flex items-center justify-between p-5">
                <div className="flex items-center gap-3">
              ...
              <div className="flex items-center justify-between p-5">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setWaitLimit(Math.max(3, waitLimit - 1))} className={cn("w-8 h-8 rounded-full font-black text-lg flex items-center justify-center", isDrivingMode ? "bg-white/10" : "bg-muted")}>−</button>
                  <span className="w-8 text-center font-black">{waitLimit}</span>
                  <button onClick={() => setWaitLimit(Math.min(15, waitLimit + 1))} className={cn("w-8 h-8 rounded-full font-black text-lg flex items-center justify-center", isDrivingMode ? "bg-white/10" : "bg-muted")}>+</button>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-3 pt-2">
          <Button
            ...
            className={cn(
              ...
              isOnline ? "border-red-500/30 text-red-500 hover:bg-red-500/10" : "border-green-500/30 text-green-500 hover:bg-green-500/10"
            )}
            onClick={() => setIsOnline(!isOnline)}
          >
            {isOnline ? "Go Offline" : "Go Online"}
          </Button>
          <Button variant="ghost" className="w-full h-14 rounded-2xl font-black uppercase tracking-widest text-sm text-red-500 hover:bg-red-500/10" onClick={handleLogout}>
